    ...
    internal readonly struct StringSegment
    {
        ...
        public StringSegment this[int start, int end]
        {
            get
            {
                ...
                if (end < 0) end += length;
                if (start < 0 || start >= length) throw new IndexOutOfRangeException();
                if (end < 0 || end >= length) throw new IndexOutOfRangeException();
        ...
        public StringSegment(string value, int start, int end)
        {
            ...
            if (start < 0 || start >= value.Length) throw new IndexOutOfRangeException();
            if (end < 0 || end >= value.Length) throw new IndexOutOfRangeException();
            this.start = start;
            this.end = end;
        }
